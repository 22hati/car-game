...
if(gameState==="play") {
  ...
  if(cactusGroup.isTouching(car)) {
    gameState="gameOver"
  }

  if(cloudsGroup.isTouching(car)) {
    score=score+1;
  ...
  textAlign(CENTER);
  textSize(40);
  text("You Lose. reload the page",0,80);
  camera.position.x=0;
  ...
  textAlign(CENTER);
  textSize(40);
  text("You WON. reload the page to play again",0,80);
  camera.position.x=0;
}
}
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    ...
    obstacle.setCollider("rectangle",0,0,20,20,90);
